...
the coding agent is an execution layer, not a discovery layer.
...
the agent to "verify its work", "double-check the output", or "use a parallel worker to
...
If implementation handoff facts are missing, emit `HANDOFF_BLOCKER` and route upstream.
