...
description: select model candidates, routing constraints, fallback behavior, and model tradeoffs for AI capabilities using task fit, quality, latency, cost, safety, modality, context, and deployment evidence. use when chatgpt needs model-selection decision memos, routing policies, fallback plans, eval requirements, deployment constraints, or low-token model decision handoffs for ai systems.
---
...
Use this skill when ChatGPT needs to choose, review, or justify model candidates for AI capabilities. Prefer connector-grounded evidence over popularity-based model choice. Use SignalDesk for local repo state and report files when available, GitHub for remote source truth, and current provider documentation or benchmark evidence only after repo facts and user-scoped requirements are captured.

When asked to produce implementation work, do not emit broad instructions. Emit a compact model decision packet or implementation handoff with exact model identifiers, routing rules, fallback posture, eval requirements, deployment constraints, validation gates, allowed and forbidden changes, halt conditions, and downstream desk targets.
...
- `references/model-selection-packet-template.md`: compact model decision packet template.
- `references/model-selection-quality-checklist.md`: quality, safety, routing, fallback, and deployment checklist.
