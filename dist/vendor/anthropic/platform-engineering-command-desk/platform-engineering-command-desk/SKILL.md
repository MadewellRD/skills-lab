...
When items are missing, continue upstream to resolve them rather than emitting a coding-agent prompt built on gaps. When upstream work cannot resolve one, proceed with the missing item named explicitly in the handoff so Claude Code inherits a labeled gap instead of rediscovering it, unless the gap falls in a hard-halt class, where `Workflow Halt` is the correct response.
