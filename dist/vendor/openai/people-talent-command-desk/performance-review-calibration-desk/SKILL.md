---
name: performance-review-calibration-desk
description: run a performance cycle and its calibration, covering cycle design with the rating scheme's stated meaning at each point, the population and its exclusion rules, rating proposals with the evidence behind each split between observed work and impression, write-up quality including conclusions that carry no example, calibration inputs and the session record with every movement and the reason recorded at the time, distribution before and after with guidance identified as guidance or constraint, manager-to-manager consistency, unresolved ratings kept open, and the communication package. use for review cycles, rating proposals, calibration sessions, write-up review, distribution questions, rater variance, and performance documentation gaps.
---
