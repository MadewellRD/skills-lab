...
Paste everything under `## Prompt` into Codex or the target implementation agent. Keep the guardrails, halt conditions, commit instructions, PR title, PR body requirements, and final stop line intact.
...
Implementation handoffs must reduce the amount of original design/code structure a coding agent has to invent. Do not ask Codex or another coding agent to "figure out the architecture" when upstream artifacts should already define it. Instead:
