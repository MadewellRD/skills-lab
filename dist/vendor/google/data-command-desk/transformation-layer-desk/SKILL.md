...
Depth standard: an artifact is complete when a builder or Jules could implement the model set from it without a follow-up round trip. An incremental model without a merge key, or a slowly changing dimension without a validity convention, is unfinished rather than draft.
