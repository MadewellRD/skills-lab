...
When the environment supports citations, cite internal files or connector results in the chat response or source-notes file. Do not put tool-specific citation tokens inside a prompt intended to be pasted into Jules unless the user explicitly wants citations in the execution prompt.
