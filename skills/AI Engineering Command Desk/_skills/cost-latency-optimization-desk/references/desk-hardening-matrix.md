...
## Cost Latency Optimization Desk expectations

`cost-latency-optimization-desk` must optimize only after baseline metrics exist. It must preserve quality, safety, grounding, privacy, and release gates while proposing model routing, caching, compression, context pruning, batching, streaming, retrieval tuning, fallback tiers, or runtime topology changes.
